...
class BookmarkAnalyzer:
    ...
    def extract_bookmarks(self, pdf_path: Path) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: List of bookmark dictionaries with title, page, and level
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if pdf_reader.outline:
                    bookmarks = self._parse_outline(pdf_reader.outline, pdf_reader)
                else:
    ...
    def _parse_outline(self, outline: Any, pdf_reader: PyPDF2.PdfReader, level: int = 0) -> List[Dict[str, Any]]:
        """
        ...
        for i, item in enumerate(outline):
            if isinstance(item, list):
                # Nested bookmarks
                bookmarks.extend(self._parse_outline(item, pdf_reader, level + 1))
            else:
                # Individual bookmark
                try:
    ...
    def _get_page_number(self, bookmark: Any, pdf_reader: PyPDF2.PdfReader) -> int:
        """
    ...
    def filter_by_level(self, bookmarks: List[Dict[str, Any]], max_level: int) -> List[Dict[str, Any]]:
        """
        ...
        self.logger.info(f"Filtered {len(filtered)} bookmarks by level <= {max_level}")
        return filtered
    
    def filter_by_keywords(self, bookmarks: List[Dict[str, Any]], keywords: List[str], 
        ...
        self.logger.info(f"Filtered {len(filtered)} bookmarks by keywords: {keywords}")
        return filtered
    
    def get_split_points(self, bookmarks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
