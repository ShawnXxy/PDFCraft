...
class PDFSplitter:
    ...
    def split_pdf(self, pdf_path: Path, split_points: List[Dict[str, Any]], 
        ...
        Args:
            ...
            split_points (List[Dict[str, Any]]): List of split point dictionaries
            ...
            with open(pdf_path, 'rb') as source_file:
                ...
                for i, split_point in enumerate(split_points):
                    try:
                        ...
                        self.logger.error(f"Error creating split PDF for '{split_point['title']}': {e}")
                        continue
                
                self.logger.info(f"Successfully created {len(split_files)} split PDF files")
                return split_files
    ...
    def _create_split_pdf(self, source_pdf: PyPDF2.PdfReader, 
                         ...
                         total_pages: int, index: int) -> Path:
        """
        ...
        Returns:
            Path: Path to created split PDF file
        """
        ...
        if start_page >= end_page:
            self.logger.warning(f"Invalid page range for '{split_point['title']}': {start_page+1}-{end_page}")
            return None
